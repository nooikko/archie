---
name: do
description: Smart orchestrator that coordinates agents, runs parallel tasks, and ensures quality at every step
user-invocable: true
---
...
Before starting, analyze the project structure:
- Check package.json to understand tech stack, scripts, and package manager
- Identify testing framework and tools
- Note any pre-commit hooks or CI/CD configurations
- Review project structure and organization patterns

Use this context to inform all decisions throughout the workflow.
...
```bash
git status
# Detect package manager and run lint if available
[npm|pnpm|yarn|bun] lint  # Use whatever package manager the project uses
```
Report any issues found.

**Note**: Adapt commands based on the project's package manager and available scripts.
...
- Backend/API (Node.js, Express, GraphQL, etc.)
- TypeScript (types, generics, interfaces, type errors)
- Testing (unit tests, integration tests, mocks, coverage)
- Architecture (system design, patterns, scaling)
- Database/Data Layer (SQL, NoSQL, ORM, etc.)
...
### 2.3 Skills Integration
Check if any installed skills should be invoked:

```
IF task requires specialized knowledge available in installed skills:
   → Check .claude/skills/ directory for available skills
   → Invoke the relevant skill before proceeding if it provides domain-specific guidance
```
...
Run available commands to verify everything is clean:
```bash
[npm|pnpm|yarn|bun] lint    # If lint script exists
[npm|pnpm|yarn|bun] test    # If test script exists
[npm|pnpm|yarn|bun] build   # If build script exists
```

**Note**: Only run scripts that exist in package.json. Adapt based on project structure.
...
Check `.claude/skills/` directory for project-specific skills. Common skills include:

| Skill | Use When | Key Capabilities |
|-------|----------|------------------|
| **frontend-design** | Frontend interfaces | Production-grade UI with high design quality |

**Note**: Skills are project-specific. Use `ls .claude/skills/` to see what's available.
